...
const TrelloCallback: React.FC = () => {
  ...
  const { loginWithTrello: authLoginWithTrello } = useAuth();
  ...
  useEffect(() => {
    const handleTrelloCallback = async () => {
      try {
        ...
        console.log('Extracted token:', trelloToken);
        
        if (!trelloToken) {
          setError('Trello token bulunamadı');
          setIsLoading(false);
          return;
        }

        // Trello kullanıcı bilgilerini al
        const trelloUser = await getTrelloUserInfo(trelloToken);
        console.log('Trello user info:', trelloUser);

        // Backend'e login isteği gönder
        const success = await authLoginWithTrello(trelloToken);
        
        if (success) {
          // Başarılı giriş - ana sayfaya yönlendir
          navigate('/home');
        } else {
          setError('Trello ile giriş başarısız');
        }
      } catch (err: any) {
        console.error('Trello callback error:', err);
        setError(err.message || 'Trello ile giriş yapılırken bir hata oluştu');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-blue-900 dark:to-indigo-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">Trello ile giriş yapılıyor...</p>
        </div>
      </div>
    );
  }
